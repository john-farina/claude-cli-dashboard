...
function getGitInfo(dir) {
  try {
    const raw = execSync("git rev-parse --abbrev-ref HEAD && git rev-parse --git-dir", {
      cwd: dir, encoding: "utf8", timeout: 3000,
      stdio: ["pipe", "pipe", "pipe"],
    }).trim();
    const lines = raw.split("\n");
    const branch = lines[0] || null;
    const gitDir = lines[1] ? path.resolve(dir, lines[1]) : "";
    const isWorktree = /\/\.git\/worktrees\//.test(gitDir);
...
function getGitInfoAsync(dir) {
  return new Promise((resolve) => {
    ...
    exec("git rev-parse --abbrev-ref HEAD && git rev-parse --git-dir", {
      cwd: dir, encoding: "utf8", timeout: 3000,
    }, (err, stdout) => {
